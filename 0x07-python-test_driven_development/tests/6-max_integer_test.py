...
class TestMaxInteger(unittest.TestCase):
    ...
    def test_max_operated_int(self):
    ...
    def test_list_loop(self):
        my_list = [1, 2, 3, 4, 5]
        self.assertEqual(max_integer([idx * 5 for idx in my_list]), 25)

    def test_one_number_in_ls(self):
        self.assertEqual(max_integer([1]), 1)

    def test_str_num_in_ls(self):
        with self.assertRaises(TypeError):
            max_integer([0, '1'])

    def test_tuple_in_ls(self):
        with self.assertRaises(TypeError):
